...
def fw_visual(simu, pred, frequency, path):
...
if __name__ == '__main__':
    ...
    wavelength = np.linespace(1000, 2000, 200)  # shape: (200,)
    para = ['para' + str(i) for i in range(4)]
    data_point = ['data' + str(i) for i in range(402)]

    simu_data = pd.read_csv(cwd+'simu/simu_data.csv', header=None, names=para+data_point)
    pred_data = pd.read_csv(cwd+'pred/pred_data.csv', header=None, names=para+data_point)
    denorm_simu_data = denorm(simu_data)
    denorm_simu_data = denorm(pred_data)
    fw_visual(denorm_simu_data, denorm_simu_data, wavelength, cwd)
